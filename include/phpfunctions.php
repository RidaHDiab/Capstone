...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="dbo.jaz_art" && $field=="id_may")
	{
		$value = $data["id_may"];;
		return $value;
	}
				if($table=="dbo.jaz_art" && $field=="id_bbc")
	{
		$value = $data["id_bbc"];;
		return $value;
	}
				if($table=="dbo.jaz_art" && $field=="BBCS")
	{
		$GetBool = DB::Query("select title as Result from bbc_art where id = '".$data["id_bbc"]."'");
$result = $GetBool->fetchAssoc();
$isNA= $result['Result'];

$value = $isNA;;
		return $value;
	}
				if($table=="dbo.jaz_art" && $field=="MS")
	{
		$GetBool = DB::Query("select title as Result from may_art where id = '".$data["id_may"]."'");
$result = $GetBool->fetchAssoc();
$isNA= $result['Result'];
